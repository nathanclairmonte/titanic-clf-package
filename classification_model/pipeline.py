...
pipeline = Pipeline(
    [
        ...
        (
            ...
            CategoricalImputer(
                imputation_method="missing", variables=config.ml_config.cat_vars
            ),
        ),
        ...
        (
            "median_imputation",
            MeanMedianImputer(
                imputation_method="median", variables=config.ml_config.num_vars
            ),
        ),
        # extract letter from each cabin entry
        (
            "letter_extractor",
            ft.ExtractLetterTransformer(variables=[config.ml_config.cabin_var]),
        ),
        ...
        (
            "rare_label_encoder",
            RareLabelEncoder(
                tol=0.05, n_categories=1, variables=config.ml_config.cat_vars
            ),
        ),
        # encode categorical variables using one-hot encoding into k-1 vars (i.e. drop last)
        (
            "categorical_encoder",
            OneHotEncoder(drop_last=True, variables=config.ml_config.cat_vars),
        ),
        # ------ SCALING ------
        ...
        (
            "clf",
            LogisticRegression(
                C=config.ml_config.log_reg_c, random_state=config.ml_config.random_state
            ),
        ),
    ]
)
